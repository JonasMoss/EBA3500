...
# Geneate uniform random variables on [0, 1].
...
# How about other number of observations?
...
# (It should be called "the central limit theorem", "det sentral grenseteoret".
...
# ... and the variances of an exponential distribution?
n_reps = 10 ** 5
n = 1000
x = rng.standard_exponential((n_reps, n))
y = np.apply_along_axis(
...
# be e.g. rng.standard_exponential). Pass the "bin" to sns.histplot.

def clt(n, func, generator, n_reps, bin = None):
...
# abs(x - median(x)). Do it for a couple of different generators chosen from 
# https://docs.scipy.org/doc/scipy/reference/stats.html, e.g. uniform, 
# standard exponential, and standard normal.
# Task (iii): Does the normal approximation work for the maximum or minimum? 
