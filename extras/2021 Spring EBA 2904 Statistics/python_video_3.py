...
means = np.apply_along_axis(func1d=np.mean, axis=1, arr=x)
...
n = 1000
x = rng.random((n_reps, n))
y = np.apply_along_axis(func1d=np.median, axis=1, arr=x)
...
n = 1000
x = rng.random((n_reps, n))
y = np.apply_along_axis(func1d=np.var, axis=1, arr=x)
mean = np.mean(y)
sd = np.sqrt(np.mean(y**2) - np.mean(y) ** 2)
xs = np.arange(mean - 3 * sd, mean + 3 * sd, 0.001)
ax = sns.histplot(x=y, stat="density")
